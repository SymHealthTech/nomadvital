...
function downloadPDF(plan) {
  const tripLabel = TRIP_TYPES.find(t => t.value === plan.tripType)?.label || plan.tripType
  const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8"/>
  <title>${plan.days}-Day ${plan.destination} Meal Plan</title>
  <style>
    body { font-family: Georgia, serif; max-width: 680px; margin: 40px auto; color: #1a1a1a; line-height: 1.7; }
    h1 { color: #085041; font-size: 22px; margin-bottom: 4px; }
    .meta { font-size: 13px; color: #666; margin-bottom: 24px; font-family: Arial, sans-serif; }
    h2 { color: #085041; font-size: 16px; margin: 20px 0 6px; font-family: Arial, sans-serif; }
    h3 { color: #1D9E75; font-size: 14px; margin: 14px 0 4px; font-family: Arial, sans-serif; }
    p { font-size: 14px; margin: 4px 0 8px; font-family: Arial, sans-serif; }
    ul { padding-left: 18px; margin: 4px 0 10px; }
    li { font-size: 14px; margin-bottom: 4px; font-family: Arial, sans-serif; }
    .footer { margin-top: 32px; padding-top: 16px; border-top: 1px solid #ccc; font-size: 11px; color: #999; font-family: Arial, sans-serif; }
    @media print { body { margin: 20px; } }
  </style>
</head>
<body>
  <h1>${plan.days}-Day ${plan.destination} Meal Plan</h1>
  <div class="meta">Condition: ${plan.condition} &nbsp;·&nbsp; Trip type: ${tripLabel} &nbsp;·&nbsp; Generated by NomadVital</div>
  ${plan.text
    .split('\n')
    .map(line => {
    ...
    .join('\n')}
  <div class="footer">General wellness information only — not medical advice. Always consult a qualified healthcare professional before making dietary changes.</div>
</body>
</html>`
    ...
    <div style={{ maxWidth: '680px', margin: '0 auto' }}>
      ...
      {plan && !loading && (
        <div style={{ background: '#fff', border: '1px solid #D3D1C7', borderRadius: '14px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ background: '#085041', padding: '16px 20px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
            <div>
            ...
            <button
              ...
              style={{
                ...
                display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, color: '#fff',
